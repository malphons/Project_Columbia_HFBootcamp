function ScatterPlot_Custom(varargin)
...
for i = 2:nargin
	...
	if strcmpi(plottype,'line')
		for k = 1:size(plotrsk,2)
			plot(sqrt(252)*plotrsk(:,k), 252*plotret(:,k), plotstyle, 'LineWidth', plotline);
			if i == 2 && k == 1
				hold on
			end
	...
	% scatter plot
		...
		if any(plotstyle == '.')
			scatter(sqrt(252)*plotrsk, 252*plotret, plotstyle);
		else
			scatter(sqrt(252)*plotrsk, 252*plotret, plotstyle, 'Filled');
		end			
		if i == 2
			hold on
		end
		if ~isempty(plotlabel)
			for k = 1:numel(plotrsk)
				if ~isempty(plotlabel{k})
					text(sqrt(252)*plotrsk(k) + 0.005, 252*plotret(k), plotlabel{k}, 'FontSize', 8);
				end
			end
		end
	end
